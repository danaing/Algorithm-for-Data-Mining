...
classification <- function() {
  ...
  repeat{
    ...
    if ((choice==4&k==2)|(choice==5&k==2)) {
      cat('If the data has more than two classes, 
          do not implement Logistic Regression and Naive Bayes. \n', 
  ...
  if (choice == 5) {
    ...
    cat("Specificity =", round(speci.t, 3), "\n\n",file = outputname, sep="", append=TRUE)
    cat("Output file has been successfully saved in ",getwd(),"/",outputname,sep="") 
  } #else warning ('Choose 1 for LDA, 2 for QDA, 3 for RDA, 4 for Logistic Regression or 5 for Naive Bayes.') 
